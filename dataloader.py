...
class DataLoader(data.Dataset):
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        for i in range(batch_size):
            # fetch image
            tmp_fc, tmp_att,\
                ix, box, tmp_wrapped = self._prefetch_process[split].get()
            if tmp_wrapped:
                wrapped = True
        ...
        if self.use_gt_box:
            fc_batch, att_batch, box_batch, label_batch, gts, infos, box_ind_batch = \
                zip(*sorted(zip(fc_batch, att_batch, box_batch, label_batch, gts, infos,box_ind_batch), key=lambda x: 0, reverse=True))
        else:
            fc_batch, att_batch, box_batch, label_batch, gts, infos = \
                zip(*sorted(zip(fc_batch, att_batch, box_batch, label_batch, gts, infos), key=lambda x: 0, reverse=True))
        data = {}
